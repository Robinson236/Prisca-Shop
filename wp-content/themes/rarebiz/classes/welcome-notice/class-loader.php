...
if( !class_exists( 'Rarebiz_Welcome_Notice' ) ){	
	class Rarebiz_Welcome_Notice{
		...
		public static function rt_welcome_ajax_action(){
			$response = array(
				'data' => array(
					'message' => esc_html__( 'Invalid Access', 'rarebiz' )
				),
				'status' => 400
			);
			...
			if( wp_verify_nonce( $_POST['nonce'], 'rt-welcome-nonce' ) ){
				...
				$response[ 'data' ][ 'message' ] = esc_html__( 'Plugin Installed and Activated Successfully', 'rarebiz' );
				$response[ 'status' ] = 200;
			}
		...
		public static function display_welcome_notice(){
			set_query_var( 'Rarebiz_Welcome_Notice_theme', self::$theme_detail );
		...
		public static function module_admin_scripts(){
			...
			$data = array(
				...
				'confirm_msg' => esc_html__( 'Are you sure?', 'rarebiz' ),
				'install_msg' => esc_html__( 'Installing...', 'rarebiz' ),
				'active_msg' => esc_html__( 'Activating...', 'rarebiz' )
			);
